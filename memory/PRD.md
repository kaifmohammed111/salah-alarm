...
- **Real full-screen alarm engine (Notifee)**: Replaced `expo-notifications` scheduling with `@notifee/react-native`. `src/lib/alarm.ts` schedules exact (`alarmManager.allowWhileIdle`) full-screen-intent alarms for every enabled prayer + pre-alarm across the next 7 days (capped 60). On fire, a full-screen ring screen (`app/alarm-ring.tsx`) opens over the lock screen (config plugin `plugins/withAlarmActivity.js` sets `showWhenLocked`/`turnScreenOn`; `USE_FULL_SCREEN_INTENT` added), plays the chosen audio ONCE (built-in tones or custom MP3 via expo-audio), vibrates, and offers swipe-to-dismiss (`SwipeToDismiss`, gesture-handler + reanimated). Silences on any hardware button / screen-off via AppState + BackHandler. Routing via `getLaunchAlarm`/foreground handler in `_layout.tsx`. NATIVE-BUILD ONLY — cannot ring in Expo Go / web preview.
- **Volume slider** memoized (`VolumeSlider`) so the 1s clock re-render no longer snaps the thumb. **Preview/Stop** button driven by explicit `previewing` flag. **Manual editor** keyboard fixed (shared local-state `TimeField`). **Asr** label no longer shows Hanafi/Shafi.
